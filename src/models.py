...
class BaseEncoder(nn.Module):
    """
    A flexible encoder model that supports different architectures.
    """
    ...
    def __init__(
        ...
        # Set feature dimension and remove classification head
        if "resnet" in arch:
        ...
        # Modify first convolutional layer for different channel counts
        if no_channels != 3:
            self._modify_first_conv(arch, no_channels)

        # Add additional block if specified
        if self.add_block:
    ...
    def _modify_first_conv(self, arch: str, no_channels: int):
...
class SigmoidGatedFusion(nn.Module):
    ...
    def __init__(self, in_dim1: int, in_dim2: int):
...
class SoftmaxGatedFusion(nn.Module):
    ...
    def __init__(self, in_dim1: int, in_dim2: int):
        ...
        self.gate = nn.Linear(in_dim1 + in_dim2, 2)

    def forward(self, x1, x2):
        combined_features = torch.cat((x1, x2), dim=1)
        weights = torch.softmax(self.gate(combined_features), dim=1)
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, in_dim1: int, in_dim2: int, shared_dim: int = 256, num_heads: int = 4):
        ...
        # Project fused features to consistent output dimension
        self.final_proj = nn.Linear(shared_dim + in_dim1 + in_dim2, 1024)
...
def get_attention_module(attention_type: str, in_dim1: int, in_dim2: int, num_heads: int = 4):
    """Factory method to return different attention mechanisms."""
...
class AQIPrediction(nn.Module):
    """Unified AQI prediction model with multi-modal inputs."""

    def __init__(
        self,
        satellite_model: Optional[BaseEncoder],
        street_model: BaseEncoder,
        ...
        if satellite_model is not None:
            ...
            if self.attention_type is not None:
                self.attention = get_attention_module(
                    self.attention_type, 
                    satellite_model.feature_dim, 
                    street_model.feature_dim
                )
        else:
            self.feature_dim = street_model.feature_dim
    ...
    def forward(self, street_img, satellite_img=None):
        ...
        if self.satellite_model is not None and satellite_img is not None:
        ...
        if self.num_classes:
            ...
            aqi = self.aqi_head(output)
            pm = self.pm_head(output)
            gas = self.gas_head(output)
            return aqi, pm, gas
